...
var pallette = [
	...
	-4972292, -9128960, -4959232, -216064, -9408400, -222052, -7864320,
	-16716800, -2565892, -16777048, -230148, -12058552, -16737124, -207684,
	-6265796, -197476, -5766916, -10445824, -5745664, 0, -10197916, -230276,
	-8650752, -16719872, -4670212, -16777052, -238340, -12582848, -16738152,
	-209744, -6528968, -197508, -6815516, -10699524, -6534144, 0, -10724260,
	-238500, -9437184, -16722944, -6513412, -16777064, -240388, -2556676,
	-16741236, -211804, -7316428, -198564, -8388404, -12537604, -2558732, 0,
	-11250604, -240556, -10223616, -16463872, -8617732, -16777080, -245508,
	-4653828, -16743292, -213860, -7841744, -197548, -9174860, -14636804,
	-4658968, 0, -11776948, -245696, -11010048, -16466944, -10723076, -16777092,
	-253700, -6488836, -16745348, -214896, -8368084, -199616, -10485604,
	-16737028, -6497060, 0, -12040120, -253920, -11796480, -16470016, -11250436,
	-16777104, -261892, -8585992, -16746376, -216960, -9155544, -199648,
	-11534204, -16741148, -9123640, 0, -12566464, -262144, -12582912, -16734208,
	-12566276, -16777116, -2096924, -10683140, -16747404, -220048, -9680860,
	-199680, -12320656, -16745268, -12009280, -6815608,
];
//}
// Decoding {
var safeAlpha =
...
function displayText(x, y, message, fontIndex, s) {
	...
	for (var i = 0; i < message.length; i++) {
		if (message[i] === ' ') {
			tempX +=
				((charWidths[fontIndex].def + charWidths[fontIndex].add) *
					2 *
					s) /
				INV_SCALE;
			continue;
...
function tryElevator(scene) {
	if (
		scene.level.worldMap[
			floor(scene.player.pos.x + scene.player.forward.x)
		...
		if (
			...
			currentLevel = scene.episode.secretExitLevel || 1;
		} else {
			currentLevel++;
		}
		return;
		...
		if (
			...
			if (
				scene.level.worldMap[secret.x + moveX][secret.y + moveY] <
					FLOOR_CODES &&
				scene.level.worldMap[secret.x + moveX * 2][
					secret.y + moveY * 2
				] < FLOOR_CODES
			) {
			...
			if (secret.temp) {
				...
				if (secret.offset <= 0) {
					...
					if (secret.tilesLeft === 0) {
						...
						secret.x += xMove;
						secret.y += yMove;
						scene.level.worldMap[secret.x + xMove][
							secret.y + yMove
						] = scene.level.worldMap[secret.x][secret.y];
...
function raycast(scene) {
	...
	var rotatedPlaneX =
		plane.x * cos(player.angle) - plane.y * sin(player.angle);
	var rotatedPlaneY =
		plane.x * sin(player.angle) + plane.y * cos(player.angle);
	...
	for (var x = 0; x < bufferWidth; x++) {
		...
		var sideDistX =
			rayDirX < 0
				? (playerX - mapX) * deltaX
				: (mapX + 1.0 - playerX) * deltaX;

		var sideDistY =
			rayDirY < 0
				? (playerY - mapY) * deltaY
				: (mapY + 1.0 - playerY) * deltaY;
						...
						if (secret.offset !== 1) {
							...
							if (side === 1) {
								if (side === secret.side) {
									...
									if (
										sideDistY - deltaY * secret.offset >=
										sideDistX
									) {
										hit = 0;
									} else if (
										...
										sideDistY +=
											deltaY * (1 - secret.offset);
									}
								} else {
									var xPos =
										playerX +
										(sideDistY - deltaY) * rayDirX;
									xPos -= floor(xPos);
								...
								if (side === secret.side) {
									...
									if (
										sideDistX - deltaX * secret.offset >=
										sideDistY
									) {
										hit = 0;
									} else if (
				...
				if (DOOR_CODES.includes(hit)) {
					...
					if (side === 1) {
						if (sideDistY - deltaY * 0.5 >= sideDistX) {
							...
							var doorX =
								playerX + (sideDistY - deltaY * 0.5) * rayDirX;
							doorX -= floor(doorX);
						...
						if (sideDistX - deltaX * 0.5 >= sideDistY) {
							...
							var doorX =
								playerY + (sideDistX - deltaX * 0.5) * rayDirY;
							doorX -= floor(doorX);
...
function renderDecorations(scene) {
	...
	for (var i = 0; i < scene.decorations.length; i++) {
		...
		var invDet =
			1 /
			(rotatedPlaneX * playerForwardY - playerForwardX * rotatedPlaneY);

		var transformY =
			invDet *
			(-rotatedPlaneY * decoration.dx + rotatedPlaneX * decoration.dy);
		...
		for (var stripe = drawStartX; stripe < drawEndX; stripe++) {
			var textureX = floor(
				((stripe - drawStartX) / decorationHeight) * 64,
			);
			if (
				stripe > 0 &&
				stripe < bufferWidth &&
...
Enemy.prototype.setTargetPosition = function (scene, x, y) {
	scene.enemyGrid[floor(this.targetPosition.x)][
		floor(this.targetPosition.y)
	] = null;
		...
		if (DOOR_CODES.includes(hit)) {
			...
			if (side === 1) {
				if (sideDistY - deltaDistY * 0.5 >= sideDistX) {
					...
					var doorX =
						this.pos.x +
						(sideDistY - deltaDistY * 0.5) * rayDirection.x;
					doorX -= floor(doorX);
				...
				if (sideDistX - deltaDistX * 0.5 >= sideDistY) {
					...
					var doorX =
						this.pos.y +
						(sideDistX - deltaDistX * 0.5) * rayDirection.y;
					doorX -= floor(doorX);
...
Enemy.prototype.ghost = function (scene) {
	...
	var dx =
		abs(scene.player.pos.x - this.pos.x) - this.definition.speed * delta;
	if (dx <= 1 + scene.player.radius) {
		var dy =
			abs(scene.player.pos.y - this.pos.y) -
			this.definition.speed * delta;
		if (dy <= 1 + scene.player.radius) {
			scene.player.damage(
				floor((2 * delta) / TICS_TO_MILLIS),
				this,
				scene,
			);
		}
	...
	for (var x = 0; x < 64; x++) {
		for (var y = 0; y < 64; y++) {
			if (scene.level.objectMap[x][y] === ENEMY_CODES_START) {
				...
				var enemyCode =
					scene.level.objectMap[x][y] - ENEMY_CODES_START - 1;
				var enemyId = -1;
				...
				if (enemyCode < 32) {
					...
					if (enemyCode < 32) {
						...
						difficultyLevel === DIFFICULTIES.I_AM_DEATH_INCARNATE
					) {
						enemyCode -= 36;
						if (enemyCode < 32) {
							enemyId = floor(enemyCode / 8);
							enemyFrame =
								enemyCode % 8 < 4 ? 'stand' : 'patrol1';
						} else if (enemyCode < 36) {
							enemyId = 4;
							enemyFrame = 'patrol1';
						}
...
Player.prototype.preUpdate = function () {
	...
	if (
		selectedWeapon < this.weapons &&
		selectedWeapon !== this.currentWeapon
	) {
...
Player.prototype.displayWeapon = function () {
	image(
		renders[
			floor(this.currentWeaponFrame + this.currentWeapon * 5) +
				weaponOffset
		],
		width / 2,
...
GameScene.prototype.draw = function () {
	switch (this.gamestate) {
		case GAME_STATES.GET_PSYCHED: // {
			...
			if (this.startTime > 0) {
				var w = constrain(
					((millis() - this.startTime) / 1000) * 440,
					0,
					440,
				);
				fill(252, 0, 0);
		...
		case GAME_STATES.SCORE_SCREEN: // {
			...
			if (this.player.floor === 10) {
				...
				if (this.killRatio === undefined) {
					...
					if (
						...
						this.timeLeft =
							this.episode.parTimes[this.player.floor - 1][0] *
								60 -
							this.seconds;
					}
				...
				if (this.fadeColor[3] === 0 || this.fadeOut) {
					if (this.timerTemp !== -1) {
						...
						if (
							floor(this.timeLeftTemp) >
							floor(this.timeLeftTemp - delta * 0.07)
						) {
							this.bonus += 500;
			...
			imageMode(CORNER);
			image(
				renders[hudOffset + 19 + this.bjBreathFrame],
				0,
				16,
				208,
				176,
			...
			if (this.startTime === -1) {
				...
				if (fizzleDone) {
					...
					for (var i = 0; i < this.enemies.length; i++) {
						if (
							Object.keys(
								this.enemies[i].definition.frames,
							).includes('deathcam')
						) {
...
MenuScene.prototype.saveGame = function () {
	...
	scopeOut('window').localStorage.setItem(
		'ep1',
		this.gamescene.episodeNumber,
	);
...
MenuScene.prototype.loadSavedGame = function () {
	...
	if (saved) {
		saveCode = saved;
		currentEpisode = parseInt(
			scopeOut('window').localStorage.getItem('ep1'),
		);
		difficultyLevel = parseInt(
			scopeOut('window').localStorage.getItem('d1'),
		);
